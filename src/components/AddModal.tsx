  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={handleClose}>
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-t-3xl md:rounded-3xl w-full md:max-w-md max-h-[92vh] overflow-y-auto shadow-xl slide-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5">
          {error && <div className="mb-4 p-3 rounded-2xl bg-red-50 dark:bg-red-900/20 text-red-500 text-sm flex items-center gap-2"><span>⚠️</span>{error}<button onClick={() => setError('')} className="ml-auto text-xs underline">关闭</button></div>}
              ...
              {mode === 'text' ? (
                <>
                  <textarea value={textInput} onChange={e => setTextInput(e.target.value)}
                    ...
                    rows={3} className="w-full px-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-violet-300"
                ...
                <div className="space-y-4">
                  {voiceText ? (
                    <div className="p-5 rounded-2xl bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 text-sm font-medium text-center">{voiceText}</div>
                  ) : (
                    <div className="text-center py-8">
                      {voiceListening ? (
                        <div className="space-y-4">
                          <div className="flex items-center justify-center gap-1.5">
                            {[1,2,3,4,5].map(i => <div key={i} className="w-1.5 bg-gradient-to-t from-violet-400 to-purple-500 rounded-full animate-pulse" style={{ height: `${14+i*8}px`, animationDelay: `${i*0.12}s`, animationDuration: '0.8s' }} />)}
                          </div>
                          <p className="text-violet-500 font-medium animate-pulse">🎤 正在聆听...</p>
                        </div>
                      ) : (
                  ...
                  {!voiceText ? (
                    <button onMouseDown={handleVoiceStart} onMouseUp={handleVoiceEnd} onMouseLeave={voiceListening ? handleVoiceEnd : undefined}
                      onTouchStart={handleVoiceStart} onTouchEnd={handleVoiceEnd}
                      className={`w-full py-14 rounded-3xl text-sm font-medium text-white transition-all select-none ${voiceListening ? 'bg-red-400 scale-95' : 'bg-gradient-to-r from-violet-500 to-purple-500 active:scale-95'}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={handleVoiceEnd} className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-500 text-white text-sm font-medium">✨ AI 解析</button>
                    </div>
                  )}
                </div>
              )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {templates.map((t, i) => (
                      <button key={i} onClick={() => { setTextInput(t.desc); handleParse() }}
                        className="px-3 py-2 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-300 hover:bg-violet-50 dark:hover:bg-violet-900/20 hover:border-violet-200 transition-all">
                        <span className="mr-1">{categories.find(c => c.name === t.catName)?.icon || '📦'}</span>{t.desc}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {step === 'parsed' && parsed && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs px-2.5 py-1 rounded-full bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400 font-medium">✨ {(parsed.confidence * 100).toFixed(0)}%</span>
              </div>
              <div className="space-y-2 bg-gradient-to-br from-gray-50 to-violet-50/50 dark:from-gray-800 dark:to-violet-900/20 rounded-2xl p-4">
                {[
                  { l: '📌 类型', v: parsed.type === 'expense' ? '支出' : '收入' },
                  { l: '💵 金额', v: `¥${parsed.amount}` },
                  { l: '🏷️ 分类', v: parsed.category },
                  { l: '📅 日期', v: parsed.date },
                  { l: '📝 描述', v: parsed.description },
                ].map(r => <div key={r.l} className="flex justify-between items-center text-sm"><span className="text-gray-400">{r.l}</span><span className="font-medium">{r.v}</span></div>)}
              </div>
              {!categories.find(c => c.name === parsed.category && c.type === parsed.type) && (
                <div className="p-3 rounded-2xl bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-xs text-yellow-700 dark:text-yellow-400">💡 「{parsed.category}」不在现有分类中，确认后将自动创建</div>
              )}
              <div className="flex gap-2">
                ...
                <button onClick={handleConfirmParsed} className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-500 text-white text-sm font-medium">✅ 确认并编辑</button>
              </div>
            </div>
          )}
          ...
          {step === 'manual' && (
            <div className="space-y-4">
              ...
              <input type="number" value={manualAmount} onChange={e => setManualAmount(e.target.value)} placeholder="0.00" step="0.01"
                className="w-full px-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 text-xl font-bold text-center focus:outline-none focus:ring-2 focus:ring-violet-300" />
              ...
              {showNewCat && (
                <div className="p-3 rounded-2xl bg-gray-50 dark:bg-gray-800 space-y-2">
                  <div className="flex gap-2">
                    <input type="text" value={newCatName} onChange={e => setNewCatName(e.target.value)} placeholder="分类名称" autoFocus onKeyDown={e => e.key === 'Enter' && handleQuickAddCat()}
              ...
              <input type="date" value={manualDate} onChange={e => setManualDate(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300" />
              <input type="text" value={manualDesc} onChange={e => setManualDesc(e.target.value)} placeholder="💬 备注（可选）" className="w-full px-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300" />

              {/* Mood picker - prominent */}
              <div>
                <p className="text-sm font-medium text-violet-600 dark:text-violet-400 mb-2 flex items-center gap-1"><span>💭</span> 此刻心情</p>
                <div className="grid grid-cols-4 gap-2">
                  {MOODS.map(m => (
                    <button key={m.value} onClick={() => setManualMood(manualMood === m.value ? '' : m.value)}
                      className={`flex flex-col items-center gap-0.5 p-2.5 rounded-2xl text-xs transition-all ${
                        manualMood === m.value ? 'bg-violet-50 dark:bg-violet-900/30 ring-2 ring-violet-400 scale-105' : 'bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700'
                      }`}>
                      <span className="text-xl">{m.emoji}</span><span className="text-[10px]">{m.label}</span>
                    </button>
                  ))}
                </div>
              </div>
