    ...
    <div className="space-y-4">
      <div className="glow-card p-5">
        ...
        <button onClick={handleGenerate} disabled={loading || stats.count === 0}
          className="w-full py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-500 text-white text-sm font-semibold disabled:opacity-50 hover:from-violet-600 hover:to-purple-600 transition-all shadow-lg shadow-violet-500/20">
          {loading ? '分析中...' : stats.count === 0 ? '暂无数据' : '生成财务报告'}
        </button>
      </div>
      {report && (
        <div className="glow-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-bold tracking-tight">报告</h3><button onClick={() => navigator.clipboard.writeText(report)} className="text-xs text-violet-500 font-medium flex items-center gap-1"><Copy size={13} />复制</button></div>
    ...
    <div className="space-y-4">
      <div className="glow-card p-5">
          ...
          <div className="space-y-3 mb-4 p-4 rounded-2xl bg-gradient-to-br from-purple-50 to-violet-50/50 dark:from-purple-900/20 dark:to-violet-900/20">
            ...
            {moodData.map(m => (
              <div key={m.label} className="flex items-center gap-2 text-sm">
                ...
                <span className="font-semibold text-violet-600 dark:text-violet-400 text-xs">{formatAmount(m.amount)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <button onClick={handleGenerate} disabled={loading || moodData.length === 0}
          className="w-full py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white text-sm font-semibold disabled:opacity-50 hover:from-violet-600 hover:to-fuchsia-600 transition-all shadow-lg shadow-violet-500/20">
          {loading ? 'AI 分析中...' : moodData.length === 0 ? '请先记录带心情的消费' : '生成心理分析报告'}
        </button>
      </div>
      {report && (
        <div className="glow-card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-bold tracking-tight">分析结果</h3><button onClick={() => navigator.clipboard.writeText(report)} className="text-xs text-violet-500 font-medium flex items-center gap-1"><Copy size={13} />复制</button></div>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-13rem)]">
      <div className="flex-1 overflow-y-auto space-y-3 mb-3">
        {messages.length === 0 ? (
          <div className="text-center py-12">
            <MessageCircle size={48} strokeWidth={1} className="text-violet-400 mx-auto mb-4" />
      ...
      <div className="flex gap-2">
        <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="问点什么..." className="flex-1 px-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur text-sm focus:outline-none focus:ring-2 focus:ring-violet-300" />
        <button onClick={handleSend} disabled={!input.trim() || loading} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-500 text-white text-sm font-semibold disabled:opacity-50 shadow-lg shadow-violet-500/20">发送</button>
      </div>
