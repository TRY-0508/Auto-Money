...
export default function Dashboard() {
  ...
  const expMoodData=useMemo(()=>{const m:Record<string,number>={};for(const t of txs){if(!t.mood||t.type!=='expense')continue;m[t.mood]=(m[t.mood]||0)+t.amount};return MOOD_LIST.filter(x=>m[x.value]).map(x=>({name:x.label,value:m[x.value]||0})).sort((a,b)=>b.value-a.value)},[txs])
  const moodColors=useMemo(()=>generateColors(Math.max(expMoodData.length,3)),[expMoodData.length])
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            ...
            <div className="card card-chart p-2 sm:p-3"><span className="text-[9px] sm:text-[10px] font-semibold">消费心情</span>
              {expMoodData.length>0?<div className="flex flex-col items-center gap-1 mt-1"><div className="w-20 h-20 sm:w-24 sm:h-24"><ResponsiveContainer><RPie><Pie data={expMoodData} cx="50%" cy="50%" innerRadius={0} outerRadius={38} paddingAngle={1} dataKey="value" stroke="#fff" strokeWidth={1.5}>{expMoodData.map((e,i)=><Cell key={i} fill={moodColors[i]}/>)}</Pie></RPie></ResponsiveContainer></div><div className="flex flex-wrap justify-center gap-x-1.5 gap-y-0.5 mt-1">{expMoodData.slice(0,4).map((e,i)=><span key={e.name} className="text-[9px] text-muted flex items-center gap-0.5"><span className="w-1.5 h-1.5 rounded-sm flex-shrink-0" style={{backgroundColor:moodColors[i]}}/>{e.name}</span>)}</div></div>:<p className="text-xs text-muted py-8">—</p>}
            </div>
          </div>
