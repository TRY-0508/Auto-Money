...
function CalendarHeatmap({ transactions, yearMonth, selectedDay, onSelectDay, onMonthChange }: {
  ...
  const dailyTotals = useMemo(() => {
    ...
    for (const t of transactions) { if (t.type === 'expense') map[t.date] = (map[t.date] || 0) + t.amount }
    return map
  }, [transactions])
  ...
  const getClass = (day: number | null, ds: string) => {
    ...
    if (r > 0.66) return 'bg-red-400 dark:bg-red-600 text-white'
    if (r > 0.33) return 'bg-orange-300 dark:bg-orange-600 text-white'
  ...
  const onTS = (e: React.TouchEvent) => { tsRef.current = e.touches[0].clientX }
  const onTE = (e: React.TouchEvent) => { const d = tsRef.current - e.changedTouches[0].clientX; if (Math.abs(d) > 60) d > 0 ? next() : prev() }

  return (
    <div className="glass rounded-3xl p-4 shadow-sm border border-white/50 dark:border-gray-800/50" onTouchStart={onTS} onTouchEnd={onTE}>
      <div className="flex items-center justify-between mb-2">
        <button onClick={prev} className="p-1 text-gray-400 hover:text-gray-600 text-lg">‹</button>
        <h3 className="text-sm font-semibold">📅 {y}年{m}月</h3>
      ...
      {weeks.map((w, wi) => (
        <div key={wi} className="grid grid-cols-7 gap-1 mb-1">
          {w.map((day, di) => {
            ...
            return <button key={di} onClick={() => onSelectDay(sel ? null : ds)} className={`aspect-square rounded-md flex items-center justify-center text-[11px] font-medium ${getClass(day, ds)} ${sel ? 'ring-2 ring-violet-500 ring-offset-1 dark:ring-offset-gray-900' : ''} ${ds === today && !sel ? 'ring-1 ring-violet-300' : ''}`}>{day}</button>
          })}
        </div>
      ))}
      <div className="flex items-center justify-center gap-2 mt-2 text-[10px] text-gray-400">
        <span>少</span><div className="w-3 h-3 rounded-sm bg-gray-100 dark:bg-gray-800" /><div className="w-3 h-3 rounded-sm bg-yellow-200 dark:bg-yellow-700" /><div className="w-3 h-3 rounded-sm bg-orange-300 dark:bg-orange-600" /><div className="w-3 h-3 rounded-sm bg-red-400 dark:bg-red-600" /><span>多</span>
        {selectedDay && <button onClick={() => onSelectDay(null)} className="ml-auto text-violet-500 text-[10px]">清除</button>}
      </div>
    </div>
  )
}
...
export default function Dashboard() {
  ...
  const moodStats = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      map[t.mood].count++; map[t.mood].totalSpent += t.amount
    }
  ...
  const budgetedCats = useMemo(() => {
    ...
    for (const c of cats) {
      ...
      if (b) totalBud += b.amount
    }
    return { totalBudget: totalBud }
  ...
  const handleEdit = (t: Transaction) => {
    ...
    setEditMood(t.mood || ''); setEditProjectId(t.projectId || '')
  }
  const handleSaveEdit = async () => {
    if (!editing) return
    const a = parseFloat(editAmount); if (!a || a <= 0) return
    await updateTransaction(editing.id, { amount: a, description: editDesc, date: editDate, categoryId: editCatId, type: editType, mood: editMood || undefined, projectId: editProjectId || undefined })
    setEditing(null)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-3 slide-up pb-4">
      ...
      <div className="flex items-center gap-2">
        <select value={yearMonth} onChange={e => setYearMonth(e.target.value)} className="px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 backdrop-blur text-sm font-medium">
        ...
        <button onClick={() => setShowCharts(!showCharts)} className="ml-auto text-xs text-violet-500 hover:text-violet-600">{showCharts ? '收起图表' : '展开图表'} {showCharts ? '▲' : '▼'}</button>
      </div>
      ...
      {showCharts && (
        <div className="rounded-3xl bg-gradient-to-br from-violet-500 via-purple-500 to-fuchsia-500 p-5 text-white shadow-lg">
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            {[
              { l: '收入', v: formatAmount(stats.totalIncome) },
              { l: '支出', v: formatAmount(stats.totalExpense) },
              { l: '结余', v: formatAmount(stats.balance) },
            ].map(c => <div key={c.l} className="bg-white/15 rounded-2xl p-3 backdrop-blur-sm"><p className="text-white/70 text-xs">{c.l}</p><p className="text-lg font-bold mt-0.5">{c.v}</p></div>)}
          </div>
          {budgetedCats.totalBudget > 0 && (
            <div className="mt-3 pt-3 border-t border-white/20">
              <div className="flex justify-between text-xs text-white/70"><span>本月预算</span><span>{formatAmount(stats.totalExpense)} / {formatAmount(budgetedCats.totalBudget)}</span></div>
              <div className="w-full h-1.5 bg-white/20 rounded-full mt-1 overflow-hidden">
                <div className={`h-full rounded-full ${stats.totalExpense > budgetedCats.totalBudget ? 'bg-red-400' : 'bg-white/50'}`} style={{ width: `${Math.min((stats.totalExpense / budgetedCats.totalBudget) * 100, 100)}%` }} />
              </div>
            </div>
          )}
        </div>
      ...
      {transactions.length === 0 ? (
        <div className="glass rounded-3xl p-8 shadow-sm border border-white/50 dark:border-gray-800/50">
          <EmptyState icon="💜" title="开始了解自己" description="记下第一笔账，同步记录心情" action={{ label: '记一笔', onClick: () => setShowAdd(true) }} />
        </div>
      ) : (
        <>
          {showCharts && (
            <>
                  ...
                  <div className="glass rounded-3xl p-4 shadow-sm border border-white/50 dark:border-gray-800/50">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-24 h-24 flex-shrink-0">
                        <ResponsiveContainer><PieChart><Pie data={moodStats} cx="50%" cy="50%" innerRadius={22} outerRadius={38} paddingAngle={2} dataKey="count"><Cell fill="#a855f7" /><Cell fill="#6366f1" /><Cell fill="#94a3b8" /><Cell fill="#38bdf8" /><Cell fill="#f97316" /><Cell fill="#ef4444" /></Pie></PieChart></ResponsiveContainer>
                      </div>
                      <div className="flex-1 space-y-1 min-w-0">{[...moodStats].slice(0, 4).map(m => <div key={m.value} className="flex items-center gap-1.5 text-xs"><span>{m.emoji}</span><span className="flex-1 truncate">{m.label}</span><span className="text-gray-400">{m.count}次</span></div>)}</div>
                    </div>
                  </div>
                  <div className="glass rounded-3xl p-4 shadow-sm border border-white/50 dark:border-gray-800/50">
                    <h3 className="text-sm font-semibold mb-2 flex items-center gap-1.5"><span>📊</span> 心情×消费额</h3>
                    <ResponsiveContainer width="100%" height={120}>
                      <BarChart data={moodStats.slice(0, 5)}><Bar dataKey="totalSpent" radius={[3, 3, 0, 0]}>{moodStats.map((_, i) => <Cell key={i} fill={MOOD_COLORS[i % MOOD_COLORS.length]} />)}</Bar></BarChart>
                    </ResponsiveContainer>
              ...
              {expenseBreakdown.length > 0 && (
                <div className="glass rounded-3xl p-4 shadow-sm border border-white/50 dark:border-gray-800/50">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-24 h-24 flex-shrink-0"><ResponsiveContainer><PieChart><Pie data={expenseBreakdown} cx="50%" cy="50%" innerRadius={22} outerRadius={40} paddingAngle={2} dataKey="amount">{expenseBreakdown.map(e => <Cell key={e.categoryId} fill={e.categoryColor} />)}</Pie></PieChart></ResponsiveContainer></div>
                    <div className="flex-1 space-y-1 min-w-0">{expenseBreakdown.slice(0, 4).map(item => <div key={item.categoryId} className="flex items-center gap-1.5 text-xs"><span>{item.categoryIcon}</span><span className="flex-1 truncate">{item.categoryName}</span><span className="text-gray-400">{item.percentage}%</span></div>)}</div>
                  </div>
                </div>
              )}
            </>
          ...
          {/* Calendar toggle + heatmap */}
          <button onClick={() => setShowCalendar(!showCalendar)} className="text-xs text-violet-500 hover:text-violet-600 text-center w-full">{showCalendar ? '收起日历 ▲' : '展开日历 ▼'}</button>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex rounded-full border border-gray-200 dark:border-gray-700 overflow-hidden">
              {(['all', 'expense', 'income'] as FilterType[]).map(t => <button key={t} onClick={() => setFilterType(t)} className={`px-3 py-1 text-xs font-medium ${filterType === t ? 'bg-violet-500 text-white' : 'bg-white/80 dark:bg-gray-900/80 text-gray-500'}`}>{t === 'all' ? '全部' : t === 'expense' ? '支出' : '收入'}</button>)}
            </div>
            <select value={filterMood} onChange={e => setFilterMood(e.target.value)} className="px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 text-xs">
              <option value="">全部心情</option>
              {MOODS.map(m => <option key={m.value} value={m.value}>{m.emoji} {m.label}</option>)}
            </select>
            <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 text-xs">
              <option value="">全部分类</option>
              {categories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
            </select>
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 text-xs" />
            <span className="text-xs text-gray-400">—</span>
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 text-xs" />
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="🔍 搜索" className="flex-1 min-w-[80px] px-3 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 text-xs focus:outline-none focus:ring-1 focus:ring-violet-400" />
          </div>
          ...
          <div className="glass rounded-3xl shadow-sm border border-white/50 dark:border-gray-800/50 overflow-hidden">
            {grouped.length === 0 ? (
              ...
              grouped.map(([date, items]) => (
                <div key={date}>
                  <div className="px-5 py-2.5 bg-gradient-to-r from-gray-50 to-transparent dark:from-gray-800/50 text-xs font-medium text-gray-500 flex justify-between">
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4 fade-in" onClick={() => setEditing(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-5 w-full max-w-sm shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <h3 className="font-semibold mb-3">✏️ 编辑记录</h3>
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">{MOODS.map(m => <button key={m.value} onClick={() => setEditMood(editMood === m.value ? '' : m.value)} className={`px-2 py-1 rounded-xl text-sm ${editMood === m.value ? 'bg-violet-50 dark:bg-violet-900/30 ring-1 ring-violet-400' : 'bg-gray-50 dark:bg-gray-800'}`}>{m.emoji} {m.label}</button>)}</div>
              </div>
      ...
      {/* FAB */}
      <button onClick={() => setShowAdd(true)} className="fixed bottom-20 md:bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-violet-500 to-purple-500 text-white rounded-full shadow-lg flex items-center justify-center text-2xl z-40 hover:scale-110 active:scale-95 transition-transform">+</button>
