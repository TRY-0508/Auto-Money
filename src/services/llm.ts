...
对于收入，收入心理学五型分类（基于自我决定理论SDT + 心理账户 + 控制点理论）：
  * 劳动收入 — 主动付出时间/技能换取的报酬（工资、兼职、稿费、加班费）
  * 增值收入 — 资产自然产生的被动回报（理财收益、利息、租金、分红）
  * 馈赠收入 — 他人出于情感/关系给予的（红包、礼物、父母资助、他人请客）
  * 惊喜收入 — 不期而遇的偶然收入（中奖、捡到钱、退税、意外退款）
  * 回流收入 — 此前支出的反向流动（报销、押金退还、退货退款）
